...
export default function BulletinPage() {
    ...
    const [goalAnimations, setGoalAnimations] = useState([]); // Array of match IDs with active goal animation
    const [lastEventTime, setLastEventTime] = useState(null); // Debug state
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'fixtures' }, (payload) => {
                ...
                // Check for score change to trigger animation
                setFixtures(currentFixtures => {
                    const oldMatch = currentFixtures.find(f => f.id === newMatch.id);

                    if (oldMatch && oldMatch.score !== newMatch.score && newMatch.status === 'LIVE') {
                        // Trigger Animation
                        setGoalAnimations(prev => [...prev, newMatch.id]);

                        // Remove animation after 3 seconds
                        setTimeout(() => {
                            setGoalAnimations(prev => prev.filter(id => id !== newMatch.id));
                        }, 3000);
                    }

                    // Update the fixture in the list
                    return currentFixtures.map(f => f.id === newMatch.id ? { ...f, ...newMatch } : f);
                });
            })
            .subscribe();
                    ...
                    sortedGroups.map((group) => (
                        <div key={group.id} className="league-group">
                            ...
                            {group.matches.map((match) => (
                                <div key={match.id} className={`match-row ${favorites.includes(match.id) ? 'favorite-row' : ''} ${goalAnimations.includes(match.id) ? 'goal-flash' : ''}`}>
                                    <Link href={`/bulletin/${match.external_id}`} className="match-link">
