...
export default function SurgeryPage() {
  ...
  const handleSubmitCoupon = async () => {
    ...
    try {
      ...
      const finalCoupon = {
        // id: Date.now(), // ID will be generated by DB
        username: currentUser.username || "misafir_cerrah",
        displayName: currentUser.first_name || currentUser.username || "Misafir Cerrah",
        rank: 1, // Mock rank
        ...
        // likes: 0, // DB default
        // comments: 0, // DB default
        // time: "Az önce", // DB default
        status: "PENDING"
      };

      // Save to Supabase
      const saved = await savePrediction(finalCoupon);

      if (saved) {
        // Success
        setCoupon([]);
        setCouponData({ confidence: 5, analysis: '' });
        alert('Kuponunuz paylaşıldı! Bol şans 🍀');
        router.push('/feed');
      } else {
        alert('Kupon paylaşılırken bir hata oluştu.');
      }
